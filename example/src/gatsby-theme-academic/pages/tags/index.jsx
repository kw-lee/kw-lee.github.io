...
import SEO from 'gatsby-theme-academic/src/components/Seo';
...
function Tags({ data }) {
  ...
  return (
    <>
      ...
      <Row gutter={[30, 20]} type="flex" className="rs-tags">
        {
          edges.map((val) => (
            <Col key={val.node.name} xs={24} sm={24} md={12} lg={8}>
              <TagCard
                img={getSrc(val.node)}
                name={val.node.name}
                description={tagData[val.node.name].description || 'No description available.'}
...
export const query = graphql`
  {
    ...
    allFile(filter: { relativeDirectory: { eq: "tags-img" } }) {
      edges {
        node {
          name
          childImageSharp {
